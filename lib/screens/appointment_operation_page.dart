...
class AppointmentOperationPage extends StatefulWidget {
  const AppointmentOperationPage({Key? key}) : super(key: key);
...
class _AppointmentOperationPageState extends State<AppointmentOperationPage> {
  ...
  String getDate() {
    if (date == null) {
      ...
      return "${date!.day <= 9 ? "0${date!.day}" : date!.day.toString()}.${date!.month <= 9 ? "0${date!.month}" : date!.month.toString()}.${date!.year}";
    }
  }
  ...
  Future appointmentListFunc() async {
    ...
    final CompanyAppointmentListJsn? appointmentNewList = await appointmentCompanyListJsnFunc(userIdData ?? 0, getDate());
    if (!mounted) return;
    setState(() {
                                          ...
                                          GestureDetector(
                                            child: Container(
                                              ...
                                              decoration: BoxDecoration(
                                                  ...
                                                  borderRadius: const BorderRadius.all(Radius.circular(minSpace))),
                                              child: const Center(
                                                child: Text("Tarihe Göre Listele",
                                                    style: TextStyle(color: Colors.black, fontSize: 17, fontFamily: contentFont)),
                                              ),
                                            ),
  ...
  Padding buildAppointmentCard(BuildContext context, int index, String customerName, String operationName, String appointmentTime, String phone,
      String appointmentDate, dynamic appointmentData) {
    return Padding(
      padding: const EdgeInsets.all(maxSpace),
                                      ...
                                      children: [
                                        ...
                                        IconButton(
                                          ...
                                          icon: Icon(LineIcons.checkSquare,
                                              color: appointmentData![index].confirmed == true ? Colors.green : primaryColor, size: iconSize),
                                          onPressed: () async {
                                            showDialog(
                                                context: context,
                                                builder: (BuildContext context) {
                                                  return ProgressHUD(
                                                    child: Builder(
                                                      builder: (context) => AlertDialog(
                                                        content: Text(
                                                            appointmentData![index].confirmed == true
                                                                ? "Randevu onayı kaldırılsın mı?"
                                                                : "Randevu onaylansın mı?",
                                                            style: const TextStyle(fontFamily: contentFont)),
                                                        actions: <Widget>[
                                                          Row(
                                                            mainAxisAlignment: MainAxisAlignment.spaceAround,
                                                            children: [
                                                              MaterialButton(
                                                                  ...
                                                                  onPressed: () async {
                                                                    final progressHUD = ProgressHUD.of(context);
                                                                    progressHUD!.show();
                                                                    final approveAppointment =
                                                                        await appointmentApproveJsnFunc(appointmentData![index].id);
                                                                    if (approveAppointment!.success == true) {
                                                                      if (!mounted) return;
                                                                      showToast(
                                                                          context,
                                                                          appointmentData![index].confirmed == true
                                                                              ? "Randevu onayı kaldırıldı!"
